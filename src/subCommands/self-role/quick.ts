...
export default new SubCommand("quick-setup", async (command) => {
    ...
    let components = [
        ...
        new MessageActionRow().addComponents(
            new MessageButton().setCustomId("previous").setLabel("◀").setStyle("PRIMARY").setDisabled(!previousPage),
    ...
    collector.on("end", async (collection) => {
        if (collection.size === 0) {
            ...
            return message.edit({ embeds, components: [] }).catch(console.error) as any
        }
